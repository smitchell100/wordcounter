...
public class WordSplitter {
    ...
    /*
    The following regex is used to split a sentence in to words.
    The following characters are considered word delimiters:
     ...
     */
    private static final String REGEX_WORD_DELIMITERS = "[\\s!?:;]|(?<=\\D),|(?<=\\d),(?=\\D)|(?<=\\d)\\.(?=\\D)|(?<=\\D)\\.(?=\\D)|(?<=\\D)\\.|(?<=\\d),(?=$)|(?<=\\d)\\.(?=$)|(?<=^)\\.|(?<=^),";



    public static List<String> wordsFromString(String textLine) {
    ...
    private static String[] splitString(String textLine) {
